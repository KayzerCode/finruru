...
$vuspot = htmlspecialchars ($_POST['vuspot']);
...
$result1 = mysql_query("UPDATE `stena` SET `name` = ('" . $newelnaz .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
$result2 = mysql_query("UPDATE `stena` SET `jm` = ('" . $jm .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
$result2 = mysql_query("UPDATE `stena` SET `vuspot` = ('" . $vuspot .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
$result3 = mysql_query("UPDATE `stena` SET `koef` = ('" . $koef .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
...
<?php include("bd.php");   ?>
<?php
$result = mysql_query("UPDATE `stena` SET `image` = '".$filename."' WHERE `id` =" .$id, $link);

$uploadfile_double = 'img/images_stena/' . $filename; // not admin folder

move_uploaded_file($_FILES['strimg']['tmp_name'], $uploadfile_double);
			
			}
$result = mysql_query ("SELECT * FROM `stena` WHERE `id`= '$id'");
$row = mysql_fetch_array ($result);
header("Location: show_stena.php?id=" . $row['id_shab'] . "&id_room=" . $id_room . "&id_stena=" . $id);
}
?>