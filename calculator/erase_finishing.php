...
if(get_magic_quotes_gpc()){ 
		...
		include ("bd.php");
		$select = mysql_query ("SELECT * FROM `components_polpokr` WHERE `id_polpokr` = '$id' AND `id_shab` = '$id_shab' AND `id_room` = '$id_room'") or die ("NOt select components_polpokr");
		while ($row = mysql_fetch_array ($select)){
			
			$res_del = mysql_query("DELETE FROM `components_polpokr` WHERE `id` = '$row[id]'") or die ("error1");
			if (!empty ($row['foto'])){
			$image = $row['id'] . '.' . 'jpg';
...
unlink ($file);}
													...
													$result = mysql_query ("SELECT * FROM `polpokr` WHERE `id` = '$id'");
								$row = mysql_fetch_array ($result);
									if (!empty ($row['image'])){
			$image = $row['id'] . '.' . 'jpg';
$file = 'img/images_polpokr/' . $image;
unlink ($file);}					
			$res_del_elem = mysql_query("DELETE FROM `polpokr` WHERE `id` = '$id'") or die ("error2");
