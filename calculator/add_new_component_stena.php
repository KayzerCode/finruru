...
<!-- Center block -->
    <tr>
        ...
        <?php if (isset ($_GET['id_stena']) AND !empty ($_GET['id_stena']))
...
if(get_magic_quotes_gpc()){ 
		...
		$id_room = (int)$id_room;
			...
			if ($id_stena == '0') {die ("Invalid group!!!!");}
			if ($id_room == '0') {die ("Invalid element!!!!");}
			
			include ("bd.php");
			
$max1 = mysql_query ("SELECT * FROM `components_stena` WHERE `id_stena` = $id_stena AND `id_room` = $id_room AND `position` = (SELECT MAX(position) FROM `components_stena` WHERE `id_stena` = $id_stena AND `id_room` = $id_room)");
$row_max1 = mysql_fetch_array ($max1);
$pos = $row_max1['position'] + 1;?>
			<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_stena.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $id_room;?>&amp;id_stena=<?php echo $id_stena;?>" title="К списку имеющихся слоёв"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="add_new_component_stena_isp.php" enctype="multipart/form-data" method="post">
<input type="hidden" name="id_stena" value="<?php echo $id_stena; ?>">
...
<tr>
	<td colspan="2" align="center">
    <input type="submit" align="middle" value="Создать" /><input type="button" value="Отмена" onclick="location.href='show_stena.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $id_room;?>&amp;id_stena=<?php echo $id_stena;?>'" title="Отменить и вернуться к списку имеющихся слоёв"/>
    </td>
    </tr>
    </tr>
</table>
