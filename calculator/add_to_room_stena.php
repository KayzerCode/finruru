...
$res = mysql_query("INSERT INTO `stena` (`id_shab`, `id_room`, `name`) VALUES ('$id_shab', '$id_room', '$row[name]')") or die ("Not insert");
$id1 = mysql_insert_id();
$image = $id1 . '.' . 'jpg';
if (!empty ($row['image']) AND file_exists ('img/images_elements/' . $id . '.' . 'jpg'))
{
$res_image = mysql_query("UPDATE `stena` SET `image` = '$image' WHERE `id` = '$id1'");
}
$result1 = mysql_query("SELECT * FROM `rooms_shab` WHERE `id` = $id_room") or die("Not select room");
$room = mysql_fetch_array($result1);

$result5 = mysql_query("UPDATE `stena` SET `jm` = '$room[jm]' WHERE `id` = '$id1'");
$resul5 = mysql_query("UPDATE `stena` SET `vuspot` = '$room[vuspot]' WHERE `id` = '$id1'");
$result6 = mysql_query("UPDATE `stena` SET `koef` = '1.0' WHERE `id` = '$id1'");
if (!empty ($row['image']) AND file_exists ('img/images_elements/' . $id . '.' . 'jpg'))
	{
	$filename = $id . '.' . 'jpg';
...
$select = mysql_query ("SELECT * FROM `components` WHERE `group` = '12' AND `elid` = '$id'") or die ("Not select components");
while ($sel = mysql_fetch_array ($select))
	{
	$res = mysql_query("INSERT INTO `components_stena` (`id_stena`, `id_shab`, `id_room`, `name`, `position`, `stmat`, `strab`, `stsopmat`, `trud`, `razm`) VALUES ('$id1', '$id_shab', '$id_room', '$sel[name]', '$sel[position]', '$sel[stmat]', '$sel[strab]', '$sel[stsopmat]', '$sel[trud]', '$sel[razm]')") or die ("Not insert comp_polpokr");
