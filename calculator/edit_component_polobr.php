...
$result = mysql_query ("SELECT * FROM `components_polobr` WHERE `id` = $id");
$row = mysql_fetch_array ($result);
?>
<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_polobr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_polobr=<?php echo $row['id_polobr']?>"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="edit_component_polobr_isp.php" enctype="multipart/form-data" method="post">
<input type="hidden" name="id" value="<?php echo $id; ?>">
<input type="hidden" name="id_room" value="<?php echo $row['id_room']; ?>">
<input type="hidden" name="id_polobr" value="<?php echo $row['id_polobr']; ?>">
...
&nbsp;&nbsp;&nbsp;<input type="submit" align="middle" value="Сохранить"/><input type="button" value="Отмена" onclick="location.href='show_polobr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_polobr=<?php echo $row['id_polobr']?>'" title="Отменить и вернуться к списку имеющихся элементов"/>
</form></div>

<?php // show picture
if (!empty ($row['foto']))
{?>
<div align="center"><?php echo "<img src=\"img/images_components_polobr/" . $row['foto'] . "\" />"; ?>
<a href="delete_foto_comp_polobr.php?id=<?php echo $row['id']; ?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div>
<?php } ?>
