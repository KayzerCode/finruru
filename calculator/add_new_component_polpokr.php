...
<!-- Center block -->
    <tr>
        ...
        <?php if (isset ($_GET['id_polpokr']) AND !empty ($_GET['id_polpokr']))
...
if(get_magic_quotes_gpc()){ 
		...
		$id_room = (int)$id_room;
			...
			if ($id_polpokr == '0') {die ("Invalid group!!!!");}
			if ($id_room == '0') {die ("Invalid element!!!!");}
			
			include ("bd.php");
			
$max1 = mysql_query ("SELECT * FROM `components_polpokr` WHERE `id_polpokr` = $id_polpokr AND `id_room` = $id_room AND `position` = (SELECT MAX(position) FROM `components_polpokr` WHERE `id_polpokr` = $id_polpokr AND `id_room` = $id_room)");
$row_max1 = mysql_fetch_array ($max1);
$pos = $row_max1['position'] + 1;?>
			<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_polpokr.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $row_max1['id_room']?>&amp;id_polpokr=<?php echo $id_polpokr;?>" title="К списку имеющихся слоёв"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="add_new_component_polpokr_isp.php" enctype="multipart/form-data" method="post">
<input type="hidden" name="id_polpokr" value="<?php echo $id_polpokr; ?>">
...
<tr>
	<td colspan="2" align="center">
    <input type="submit" align="middle" value="Создать" /><input type="button" value="Отмена" onclick="location.href='show_polpokr.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $row_max1['id_room']?>&amp;id_polpokr=<?php echo $id_polpokr;?>'" title="Отменить и вернуться к списку имеющихся слоёв"/>
    </td>
    </tr>
    </tr>
</table>
