...
$result = mysql_query("SELECT * FROM `potobr` WHERE `id` = $id", $link);
	$row = mysql_fetch_array ($result);
			?>
			<table cellspacing="10" align="center" width="900">
	<tr>
    	<td>
        	<div align="left"><a href="show_potobr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room']?>&amp;id_potobr=<?php echo $id;?>" title="Вернуться назад"><?php include ("button_back.php");?></a></div>
        </td>
       
    </tr>
</table>
<div align="center"><form action="edit_element_potobr_isp.php" enctype="multipart/form-data" method="post">
...
<div align="center"><input type="file" name="strimg"></div>
     ...
     <tr>
     	<td colspan="2">
<div align="center"><input type="submit" align="middle" value="Редактировать"/><input type="button" value="Отмена" onclick="location.href='show_potobr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room']?>&amp;id_potobr=<?php echo $id;?>'" title="Вернуться назад"/></div>
		</td>
     </tr>
</table><br />
</form>
...
if (!empty ($row['image']))
{?>
<div align="center"><?php echo "<img src=\"img/images_potobr/" . $row['image'] . "\" />"; ?>
<a href="delete_foto_potobr.php?id=<?php echo $row['id']; ?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div><br />
<?php } ?>
