...
<title>Смотреть и редактировать параметры элемента отделки потолка</title>
...
$_GET['id_potpokr'] = htmlspecialchars ($_GET['id_potpokr']);

if(get_magic_quotes_gpc()){ 
            $id = stripslashes($_GET['id']);
			            $id_room = stripslashes($_GET['id_room']); 
			            $id_potpokr = stripslashes($_GET['id_potpokr']); 

        } else { 
            $id = addslashes($_GET['id']);
			            $id_room = addslashes($_GET['id_room']); 
			            $id_potpokr = addslashes($_GET['id_potpokr']); 
        ...
        $id = (int)$id;
			...
			$res = mysql_query ("SELECT * FROM `potpokr` WHERE `id` = '$id_potpokr'");
...
<tr>
<td>
<div align="left"><font size="+2"><?php echo $row['name']; ?></font></div></td><td><div align="right"><a href="edit_element_potpokr.php?id=<?php echo $row['id'];?>" title="Редактировать элемент"><?php include ("button_edit.php");  // the button edit?></a></div></td><td><div align="left"><a href="delete_element_potpokr.php?id=<?php echo $row['id'];?>" title="Удалить элемент"><?php include ("button_delete.php");  // the button delete?></a></div>
</td>
<td><div align="right"><a href="add_new_component_potpokr.php?id_potpokr=<?php echo $row['id'];?>&amp;id_room=<?php echo $id_room;?>" title="Добавить новый слой"><?php include ("button_add_component.php"); // the button ADD COMPONENT?> </a></div></td>
</tr>
</table>
...
$total_trud = 0;
$total = 0;
$result1 = mysql_query("SELECT * FROM `components_potpokr` WHERE `id_potpokr` = '$id_potpokr' AND `id_shab` = '$row[id_shab]' AND `id_room` = '$id_room' ORDER BY `position` ");
$max = mysql_query ("SELECT * FROM `components_potpokr` WHERE `id_potpokr` = '$id_potpokr' AND `id_shab` = '$row[id_shab]' AND `id_room` = '$id_room' AND `position` = (SELECT MAX(position) FROM `components_potpokr` WHERE `id_potpokr` = '$id_potpokr' AND `id_shab` = '$row[id_shab]' AND `id_room` = '$id_room')");
$min = mysql_query ("SELECT * FROM `components_potpokr` WHERE `id_potpokr` = '$id_potpokr' AND `id_shab` = '$row[id_shab]' AND `id_room` = '$id_room' AND `position` = (SELECT MIN(position) FROM `components_potpokr` WHERE `id_potpokr` = '$id_potpokr' AND `id_shab` = '$row[id_shab]' AND `id_room` = '$id_room')");
		...
		?>
				<tr bordercolor="#9999CC">
                	<td id="comp">
						...
						<a href="up_component_potpokr.php?id_up_comp=<?php echo $row1['id'];?>" title="Поднять слой"><?php include ("button_up.php"); } // button up ?> </a>
                    </td>
                    
                    <td id="comp">
                    	<?php if ( $row1['position'] < $row_max['position'] ) { ?> 
						<a href="down_component_potpokr.php?id_down_comp=<?php echo $row1['id'];?>" title="Опустить слой"><?php include ("button_down.php"); } // button down?></a>
                    </td>
                    
                    <td id="comp">
                    	<a href="show_component_potpokr.php?show_comp_id=<?php echo $row1['id'];?>&amp;id=<?php echo $row1['id_shab']?>"><?php echo $row1['name'];?></a>
                    </td>
                    ...
                    <td id="comp">
                    	<a href="edit_component_potpokr.php?id_edit_comp=<?php echo $row1['id'];?>" title="Редактировать"><?php include ("button_edit_component.php"); // button edit component?></a>
                    </td>
                    
                    <td id="comp">
                    	<a href="delete_component_potpokr.php?id_del_comp=<?php echo $row1['id'];?>" title="Удалить"><?php include ("button_delete_component.php"); // button deletу component?></a>
                    </td>
                </tr><?php
...
if (!empty ($row['image']))
{?>
<br /><div align="center"><?php echo "<img src=\"img/images_potpokr/" . $row['image'] . "\" title=\"Фото элемента\"/>"; ?>
</div><br />
<?php } 
