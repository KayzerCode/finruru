...
if(get_magic_quotes_gpc()){ 
		...
		include ("bd.php");
		$select = mysql_query ("SELECT * FROM `components_stena` WHERE `id_stena` = '$id' AND `id_shab` = '$id_shab' AND `id_room` = '$id_room'") or die ("NOt select components_polpokr");
		while ($row = mysql_fetch_array ($select)){
			
			$res_del = mysql_query("DELETE FROM `components_stena` WHERE `id` = '$row[id]'") or die ("error1");
			if (!empty ($row['foto'])){
			$image = $row['id'] . '.' . 'jpg';
...
unlink ($file);}					
			$res_del_elem = mysql_query("DELETE FROM `stena` WHERE `id` = '$id'") or die ("error2");
