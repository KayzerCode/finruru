...
$result1 = mysql_query("UPDATE `potpokr` SET `name` = ('" . $newelnaz .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
$result2 = mysql_query("UPDATE `potpokr` SET `metraj` = ('" . $metraj .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
$result3 = mysql_query("UPDATE `potpokr` SET `koef` = ('" . $koef .  "') WHERE id = ('" . $id .  "')", $link) or die ("error");
...
<?php include("bd.php");   ?>
<?php
$result = mysql_query("UPDATE `potpokr` SET `image` = '".$filename."' WHERE `id` =" .$id, $link);

$uploadfile_double = 'img/images_potpokr/' . $filename; // not admin folder

move_uploaded_file($_FILES['strimg']['tmp_name'], $uploadfile_double);
			
			}

header("Location: show_potpokr.php?id=" . $id_shab . "&id_room=" . $id_room . "&id_potpokr=" . $id);
}
?>