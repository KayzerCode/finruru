...
$result = mysql_query ("SELECT * FROM `components_polpokr` WHERE `id` = $id");
$row = mysql_fetch_array ($result);
?>
<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_polpokr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_polpokr=<?php echo $row['id_polpokr']?>"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="edit_component_polpokr_isp.php" enctype="multipart/form-data" method="post">
<input type="hidden" name="id" value="<?php echo $id; ?>">
<input type="hidden" name="id_room" value="<?php echo $row['id_room']; ?>">
<input type="hidden" name="id_polpokr" value="<?php echo $row['id_polpokr']; ?>">
...
&nbsp;&nbsp;&nbsp;<input type="submit" align="middle" value="Сохранить"/><input type="button" value="Отмена" onclick="location.href='show_polpokr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_polpokr=<?php echo $row['id_polpokr']?>'" title="Отменить и вернуться к списку имеющихся элементов"/>
</form></div>

<?php // show picture
if (!empty ($row['foto']))
{?>
<div align="center"><?php echo "<img src=\"img/images_components_polpokr/" . $row['foto'] . "\" />"; ?>
<a href="delete_foto_comp_polpokr.php?id=<?php echo $row['id']; ?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div>
<?php } ?>
