...
if (isset ($_GET['id_down_comp']) AND !empty ($_GET['id_down_comp']))
	...
	$_GET['id_down_comp'] = htmlspecialchars ($_GET['id_down_comp']);
		...
		$result2 = mysql_query ("SELECT * FROM `components_potpokr` WHERE `position` = (SELECT MIN(position) FROM `components_potpokr` WHERE `position` > '$row1[position]' AND `id_shab` = $row1[id_shab] AND `id_potpokr` = $row1[id_potpokr] AND `id_room` = $row1[id_room]) AND `id_shab` = $row1[id_shab] AND `id_potpokr` = $row1[id_potpokr] AND `id_room` = $row1[id_room]");
