...
<!-- Center block -->
    <tr>
        ...
        <?php if (isset ($_GET['id_potpokr']) AND !empty ($_GET['id_potpokr']))
...
if(get_magic_quotes_gpc()){ 
		...
		$id_room = (int)$id_room;
			...
			if ($id_potpokr == '0') {die ("Invalid group!!!!");}
			if ($id_room == '0') {die ("Invalid element!!!!");}
			
			include ("bd.php");
			
$max1 = mysql_query ("SELECT * FROM `components_potpokr` WHERE `id_potpokr` = $id_potpokr AND `id_room` = $id_room AND `position` = (SELECT MAX(position) FROM `components_potpokr` WHERE `id_potpokr` = $id_potpokr AND `id_room` = $id_room)");
$row_max1 = mysql_fetch_array ($max1);
$pos = $row_max1['position'] + 1;?>
			<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_potpokr.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $row_max1['id_room']?>&amp;id_potpokr=<?php echo $id_potpokr;?>" title="К списку имеющихся слоёв"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="add_new_component_potpokr_isp.php" enctype="multipart/form-data" method="post">
<input type="hidden" name="id_potpokr" value="<?php echo $id_potpokr; ?>">
...
<tr>
	<td colspan="2" align="center">
    <input type="submit" align="middle" value="Создать" /><input type="button" value="Отмена" onclick="location.href='show_potpokr.php?id=<?php echo $row_max1['id_shab'];?>&amp;id_room=<?php echo $row_max1['id_room']?>&amp;id_potpokr=<?php echo $id_potpokr;?>'" title="Отменить и вернуться к списку имеющихся слоёв"/>
    </td>
    </tr>
    </tr>
</table>
