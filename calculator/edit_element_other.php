...
$result = mysql_query("SELECT * FROM `handbook_elements` WHERE `id` = $elid", $link);
	$row = mysql_fetch_array ($result);
			?>
			<table cellspacing="10" align="center" width="900">
	<tr>
    	<td>
        	<div align="left"><a href="add_elements.php?id_shab=<?php echo $id_shab;?>&amp;group=<?php echo $group;?>" title="Назад к списку элементов"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="edit_element_other_isp.php" enctype="multipart/form-data" method="post">
...
&nbsp;&nbsp;&nbsp;<input type="submit" align="middle" value="Редактировать"/><input type="button" value="Отмена" onclick="location.href='add_elements.php?id_shab=<?php echo $id_shab;?>&amp;group=<?php echo $group;?>'" title="Отменить и вернуться к списку элементов"/>
</form></div><br /><br /><br />
...
<a href="delete_foto_elem_other.php?id_shab=<?php echo $id_shab;?>&amp;group=<?php echo $group;?>&amp;elid=<?php echo $elid;?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div><br />
<?php } ?>
