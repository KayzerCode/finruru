...
$result = mysql_query ("SELECT * FROM `shab_components` WHERE `id` = $id");
$row = mysql_fetch_array ($result);
?>
<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_element_other.php?id=<?php echo $row['id_shab'];?>&amp;elid=<?php echo $row['elid'];?>&amp;group=<?php echo $row['group']?>"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="edit_component_shablon_isp.php" enctype="multipart/form-data" method="post">
...
&nbsp;&nbsp;&nbsp;<input type="submit" align="middle" value="Сохранить"/><input type="button" value="Отмена" onclick="location.href='show_element_other.php?id=<?php echo $row['id_shab'];?>&amp;elid=<?php echo $row['elid'];?>&amp;group=<?php echo $row['group']?>'" title="Отменить и вернуться к списку имеющихся элементов"/>
</form></div>

<?php // show picture
if (!empty ($row['foto']))
{?>
<div align="center"><?php echo "<img src=\"img/images_components_shablon/" . $row['foto'] . "\" />"; ?>
<a href="delete_foto_comp_shablon.php?id=<?php echo $id; ?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div>
<?php } ?>
