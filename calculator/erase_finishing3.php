...
if(get_magic_quotes_gpc()){ 
		...
		include ("bd.php");
		$select = mysql_query ("SELECT * FROM `components_potpokr` WHERE `id_potpokr` = '$id' AND `id_shab` = '$id_shab' AND `id_room` = '$id_room'") or die ("NOt select components_polpokr");
		while ($row = mysql_fetch_array ($select)){
			
			$res_del = mysql_query("DELETE FROM `components_potpokr` WHERE `id` = '$row[id]'") or die ("error1");
			if (!empty ($row['foto'])){
			$image = $row['id'] . '.' . 'jpg';
$file = 'img/images_components_potpokr/' . $image;
unlink ($file);}
													}
													$result = mysql_query ("SELECT * FROM `potpokr` WHERE `id` = '$id'");
								$row = mysql_fetch_array ($result);
									if (!empty ($row['image'])){
			$image = $row['id'] . '.' . 'jpg';
$file = 'img/images_potpokr/' . $image;
unlink ($file);}					
			$res_del_elem = mysql_query("DELETE FROM `potpokr` WHERE `id` = '$id'") or die ("error2");
