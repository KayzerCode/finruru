...
$result = mysql_query ("SELECT * FROM `components_potpokr` WHERE `id` = $id");
$row = mysql_fetch_array ($result);
?>
<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_potpokr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_potpokr=<?php echo $row['id_potpokr']?>"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="edit_component_potpokr_isp.php" enctype="multipart/form-data" method="post">
...
&nbsp;&nbsp;&nbsp;<input type="submit" align="middle" value="Сохранить"/><input type="button" value="Отмена" onclick="location.href='show_potpokr.php?id=<?php echo $row['id_shab'];?>&amp;id_room=<?php echo $row['id_room'];?>&amp;id_potpokr=<?php echo $row['id_potpokr']?>'" title="Отменить и вернуться к списку имеющихся элементов"/>
</form></div>

<?php // show picture
if (!empty ($row['foto']))
{?>
<div align="center"><?php echo "<img src=\"img/images_components_potpokr/" . $row['foto'] . "\" />"; ?>
<a href="delete_foto_comp_potpokr.php?id=<?php echo $row['id']; ?>" title="Удалить картинку"><?php include ("button_delete_component.php");?></a></div>
<?php } ?>
