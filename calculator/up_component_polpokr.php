...
if (isset ($_GET['id_up_comp']) AND !empty ($_GET['id_up_comp']))
	...
	$_GET['id_up_comp'] = htmlspecialchars ($_GET['id_up_comp']);
		...
		$result2 = mysql_query ("SELECT * FROM `components_polpokr` WHERE `position` = (SELECT MAX(position) FROM `components_polpokr` WHERE `position` < '$row1[position]' AND `id_shab` = $row1[id_shab] AND `id_polpokr` = $row1[id_polpokr] AND `id_room` = $row1[id_room]) AND `id_shab` = $row1[id_shab] AND `id_polpokr` = $row1[id_polpokr] AND `id_room` = $row1[id_room]");
