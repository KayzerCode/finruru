...
if (file_exists ('img/images_potobr/' . $id . '.' . 'jpg'))
{
	unlink ('img/images_potobr/' . $id . '.' . 'jpg');
}
$res_del = mysql_query("DELETE FROM `potobr` WHERE `id` = $id") or die ("error");
$res = mysql_query ("SELECT * FROM `components_potobr` WHERE `id_potobr` = '$id'");
while ($row2 = mysql_fetch_array ($res))
	{
		if (file_exists ('img/images_components_potobr/' . $row2['id'] . '.' . 'jpg'))
{
	unlink ('img/images_components_potobr/' . $row2['id'] . '.' . 'jpg');
}
		$res_del2 = mysql_query("DELETE FROM `components_potobr` WHERE `id` = $row2[id]") or die ("error");
