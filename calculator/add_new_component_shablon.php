...
$max1 = mysql_query ("SELECT * FROM `shab_components` WHERE `id_shab` = '$id' AND `elid` = $elid AND `position` = (SELECT MAX(position) FROM `shab_components` WHERE `id_shab` = '$id' AND `elid` = $elid)");
$row_max1 = mysql_fetch_array ($max1);
$pos = $row_max1['position'] + 1;?>
			<table cellspacing="10">
	<tr>
    	<td>
        	<div align="left"><a href="show_element_other.php?id=<?php echo $id;?>&amp;elid=<?php echo $elid;?>&amp;group=<?php echo $group;?>" title="К списку имеющихся слоёв"><?php include ("button_back.php");?></a></div>
        </td>
    </tr>
</table>
<div align="center"><form action="add_new_component_shablon_isp.php" enctype="multipart/form-data" method="post">
...
<tr>
	<td colspan="2" align="center">
    <input type="submit" align="middle" value="Создать" /><input type="button" value="Отмена" onclick="location.href='show_element_other.php?id=<?php echo $id;?>&amp;elid=<?php echo $elid;?>&amp;group=<?php echo $group;?>'" title="Отменить и вернуться к списку имеющихся слоёв"/>
    </td>
    </tr>
    </tr>
</table>
